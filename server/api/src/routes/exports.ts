...
import { getQuickFacts } from '@outbound/core';
...
const BASE_DIM_KEYS = TVI_DIMENSIONS.filter((d) => !d.isComposite).map((d) => d.key);
...
async function loadExportBundle(
  ...
  const overall =
    ...
    (tvi?.overall_score != null ? Number(tvi.overall_score) : null);
  ...
  const trends = BASE_DIM_KEYS.map((key) => {
    const meta = TVI_DIMENSIONS.find((d) => d.key === key);
  ...
  const sourcesRaw = Array.isArray(tvi?.sources) ? tvi!.sources : [];
  ...
  const freshness = tvi?.data_freshness
    ? new Date(tvi.data_freshness).toISOString().slice(0, 10)
    : null;
  ...
  return {
    ...
    overall,
    confidence: tvi?.confidence ?? null,
...
function bundleToCsvRow(b: ExportBundle): (string | number | null)[] {
  ...
  return [
    ...
    b.confidence,
    d.tourismInfrastructure ?? null,
    d.accessibility ?? null,
    d.costIndex ?? null,
    d.safetyAndEntry ?? null,
    d.travelInfrastructure ?? null,
    d.crowding ?? null,
    d.trajectory ?? null,
...
function buildPdf(bundle: ExportBundle): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(18).fillColor('#111').text('Outbound — Travel Viability Report', {
      align: 'left',
    });
    doc.moveDown(0.3);
    doc
    ...
    doc
      ...
      .fillColor('#666')
      .text(
        `Generated: ${stamp} · Scoring engine v${TVI_SCORING_VERSION}`,
        { align: 'left' }
      );
    doc.moveDown(1);

    doc.fontSize(12).fillColor('#111').text('TRAVEL VIABILITY INDEX');
    doc.moveDown(0.3);
    doc
      .fontSize(10)
      .fillColor('#333')
    ...
    for (const dim of TVI_DIMENSIONS) {
    ...
    doc
      ...
      .fillColor('#888')
      .text(
        'Generated by Outbound · outbound.io · TVI methodology: outbound.io/docs/methodology',
        { align: 'center' }
      );
...
router.get('/geography/:id/pdf', optionalAuth, requireTier('pro'), async (req: Request, res: Response) => {
  try {
    ...
    const filename = `Outbound_${iso}_${formatDateStamp()}.pdf`;
...
router.get('/geography/:id/csv', optionalAuth, requireTier('pro'), async (req: Request, res: Response) => {
  try {
    ...
    const iso = bundle.isoCode ?? 'GEO';
    const filename = `Outbound_${iso}_${formatDateStamp()}.csv`;
...
router.get('/compare/csv', optionalAuth, requireTier('pro'), async (req: Request, res: Response) => {
  try {
    ...
    const filename = `Outbound_compare_${isos.join('-')}_${formatDateStamp()}.csv`;
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${filename}"`
