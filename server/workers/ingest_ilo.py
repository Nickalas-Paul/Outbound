"""
Ingest ILO labor-market indicators into raw_indicators via the World Bank API.

ILOSTAT series are published through the standard World Bank Indicators API:
  https://api.worldbank.org/v2/country/all/indicator/{code}?format=json&per_page=500&date={range}
...
def configure_logging() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(LOGS_DIR / "ingest_ilo.log", encoding="utf-8"),
        ],
    )


logger = logging.getLogger("ingest_ilo")
...
if __name__ == "__main__":
    ...
    try:
        ingest()
        logger.info("ingest_ilo completed successfully")
    except Exception:
        logger.exception("ingest_ilo failed")
        sys.exit(1)
