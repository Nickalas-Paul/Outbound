"""
Ingest World Bank education indicators into raw_indicators (global coverage).
...
INDICATORS = [
    (
        "SE.TER.CUAT.BA.ZS",
        "Educational attainment, at least Bachelor's or equivalent (% of population 25+)",
        "percent",
    ),
    (
        "SE.TER.ENRR",
    ...
    (
        "SL.TLF.ADVN.ZS",
        "Labor force with advanced education (% of total working-age population)",
        "percent",
    ),
    (
        "SE.XPD.TOTL.GD.ZS",
...
def configure_logging() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(LOGS_DIR / "ingest_education.log", encoding="utf-8"),
        ],
    )


logger = logging.getLogger("ingest_education")
...
if __name__ == "__main__":
    ...
    try:
        ingest()
        logger.info("ingest_education completed successfully")
    except Exception:
        logger.exception("ingest_education failed")
        sys.exit(1)
